...
- canonical default table-view columns and visible-column order
...
- `GH-SETUP-006A` Bootstrap docs require `Project Priority` (`P0` through `P4`), `Order` (number), `Priority` (`Urgent`, `High`, `Medium`, `Low`), and `Size` (`XS`, `S`, `M`, `L`, `XL`) as canonical board fields.
...
- `GH-SETUP-006C` Bootstrap docs require the default table view to show `Title`, `Assignees`, `Status`, `Project Priority`, `Order`, `Priority`, and `Repository` in that order for both new and adopted boards.
- `GH-SETUP-006D` Bootstrap docs prefer copying a configured canonical template for new boards and require exact tracked remediation when an adopted board's view cannot be changed through an available supported capability.
...
## Verification
Verification is documentation-driven. Success requires GitHub bootstrap behavior to be explicit enough that a bootstrap run can determine one canonical board target, normalize its fields and default table view, link the repo, report current state accurately, distinguish repaired history from final live state, and leave backlog ordering explicit for automation through `Project Priority` and `Order`.
